...
.controller('GutscheineCtrl', function($scope, $stateParams, $rootScope, $ionicLoading, Friends) {
    ...
    $scope.city = $stateParams.city;
		...
		$scope.$on('$ionicView.afterEnter', function(){
			$ionicLoading.show({
				template: '<i class="icon ion-loading-a"></i><br>Aktualisiere Gutscheine...',
				noBackdrop: true
			});
...
.controller('GutscheinDetailCtrl', function($scope, $rootScope, $http, $state, $stateParams, $ionicLoading, $ionicPopup, Friends) {
    ...
    Friends.getDetail($stateParams.friendId).then(function(b) {
		...
		$scope.doRefreshDetailLoading = function() {
			$ionicLoading.show({
				template: '<i class="icon ion-loading-a"></i><br>Aktualisiere Details...',
				noBackdrop: true
			});
        ...
        confirmPopup.then(function(res) {
						...
						if(!res){
							...
							var alertPopup = $ionicPopup.alert({
								title: 'Gutschein schon eingelöst',
								template: '<i id="false" class="icon ion-close"></i>',
								okType: "button-calm"
							});
						}else if(check == "false") {
                ...
                $ionicLoading.hide();
                var alertPopup = $ionicPopup.alert({
                    title: 'Code falsch oder Gutschein abgelaufen',
                    template: '<i id="false" class="icon ion-close"></i>',
                    okType: "button-calm"
                });
                    ...
                    if (response4.data.length == 0) {
                        ...
                        if (!ok) {
                            ...
                            var alertPopup = $ionicPopup.alert({
                                ...
                                template: '<i id="false" class="icon ion-checkmark"></i>',
                                okType: "button-calm"
                            });
